...
class ActivityHelper:
    ...
    async def fetch_server(self, serverid: int) -> tuple | Row:
        ...
        await cur.execute("SELECT * FROM activity WHERE ServerID = ?", (serverid,))
    ...
    async def fetch_users(self, serverid: int) -> tuple | Row:
        ...
        """
        cur: aiosqlite.Cursor = await self.bot.act_database.cursor()
        await cur.execute("SELECT * FROM Users WHERE ServerID = ?", (serverid,))
    ...
    async def create_act_image(self, member: disnake.Member):
        ...
        circle_mask = logo_mask.resize(logo.size, Image.LANCZOS)
        logo.putalpha(circle_mask)

        image.paste(logo, (20, 20), mask=logo)
        font = ImageFont.truetype("assets/fonts/Exo-Regular.otf", 48)
        smallFont = ImageFont.truetype("assets/fonts/Exo-Regular.otf", 32)
        ImageDraw.Draw(image).text(
            xy=(logo.size[0] + 20, logo.size[1] - 90),
            text=f"{member.name}",
            font=font,
        )
        ImageDraw.Draw(image).rounded_rectangle(
            xy=((45, 180), (400, 300)), radius=7, width=3
        )
        ImageDraw.Draw(image).rectangle(xy=((50, 185), (395, 295)), fill="black")
        ImageDraw.Draw(image).text(
            xy=(50, 200), text=f"ACT Points: {user[2]}", font=smallFont, fill="white"
        )
        ImageDraw.Draw(image).text(
            xy=(50, 240), text=f"Total msg: {user[4]}", font=smallFont, fill="white"
        )

        ImageDraw.Draw(image).rounded_rectangle(
            xy=((50, 300), (500, 350)), radius=7, width=3
        )
        # 495 = 100%
        ...
        ImageDraw.Draw(image).rectangle(xy=((55, 305), (percent, 345)), fill="white")
    ...
    async def set_time(self, serverid: int):
        ...
        if previous_reset:
            ...
            await self.insert_into("Time", values=(serverid, next_reset_epoch))
    ...
    async def fetch_embed(self, serverid: int) -> disnake.Embed | None:
        ...
        embed = disnake.Embed(title=embed_raw[1], description=embed_raw[2])
    ...
    async def select_one(
        ...
        column (str, optional): The column that you need to be selected. \
        Defaults to None.
        filter_column (str | None, optional): The column in which you specify
        the value that disqualifies a row . Defaults to None.
        filter_value (any, optional): The value that you need to filtered. Defaults\
        to None.
        """
        ...
        cur: aiosqlite.Cursor = await self.bot.act_database.cursor()
        await cur.execute(
            f"SELECT {column} FROM {table} {where_statement}", filter_values
        )
        result = await cur.fetchone()
        return result
    ...
    async def update_set(
        ...
        await cur.execute(f"UPDATE {table} {set_statement} {where_statement}", endval)
        await self.bot.act_database.commit()
