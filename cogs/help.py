...
class MyHelp(commands.HelpCommand):
    ...
    async def send_command_help(self, command):
        ...
        embed.add_field(
            name="⦑ Name ⦒", value=f"`{newcommand.qualified_name}`", inline=False
        )
        ...
        embed.add_field(
            name="⦑ usage ⦒",
            value=f"`?{newcommand.name + newcommand.signature}`"
            if newcommand.signature
            else f"`?{newcommand.name}`",
            inline=False,
        )
        embed.add_field(
            name="⦑ Description ⦒", value=f"{newcommand.description}", inline=False
        )
